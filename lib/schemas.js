...
function assertSequelizeModel(model, msg) {
  if (model === undefined || model === null) throw Error(msg || "Model should be instance of Sequelize Model, instead of ".concat(model.constructor.name));
}
/*
...
function schema(sequelize) {
  ...
  assertNotUndefined(sequelize, "schema function should receive a sequelize instance, received ".concat(_typeof(sequelize)));
  return [generateInputOperators(sequelize), generateInputWhere(sequelize), generateInputCreate(sequelize), generateInputUpdate(sequelize), generateTypeModels(sequelize), generateQueries(sequelize), generateMutations(sequelize), generateSubscriptions(sequelize), extend].join('\n');
}
...
function generateInputOperators(sequelize) {
  ...
  var modelsTypes = models.reduce(function (acc, model) {
    ...
    Object.values(model.rawAttributes).map(function (attribute) {
...
function generateInputWhere(sequelize) {
  ...
  var modelsWheres = models.reduce(function (acc, model) {
    ...
    Object.values(model.rawAttributes).map(function (attribute) {
      if (attribute.type instanceof _sequelize["default"].VIRTUAL) return acc;
...
function generateTypeModels(sequelize) {
  ...
  var modelsTypesAssociations = models.reduce(function (acc, model) {
    var modelName = getModelName(model);
    if (!acc[modelName]) acc[modelName] = {};
    Object.values(model.associations).map(function (association) {
